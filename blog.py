...
secret = "dvamefFVAEWfdsnclaSFvjudfaVfnas"
...
class Post(db.Model):
    ...
    def render(self):
        ...
        return render_str("postc.html", p = self)
...
class Permalink(BlogHandler):
    def get(self,entry_id):
        ...
        if not post:
            ...
            return
        if self.format == 'html':
            self.render("success.html", entry=post,age=age_str(age))
...
app = webapp2.WSGIApplication([('/([0-9]+)(?:.json)?',Permalink),
                               ('/?(?:.json)?', MainPage),
                               ('/flush', FlushHandler),
                               ('/newpost',SubmitPost),
                               ('/signup',Register),
